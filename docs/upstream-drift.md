# Upstream drift manifest for devportal-core
...
- `main` is the default branch and the product line: every image from
  `3.0.0-alpha.5` on, including `:edge`, builds from it. Our drift over
  upstream lives here, including the append-only Containerfile blocks and the
  `veecode/` files listed below.
- `upstream/main` is the untouched mirror of `redhat-developer/rhdh` `main`
  that used to be called `main`.
- `veecode/release-1.10` tracks upstream `release-1.10` and is frozen at
  `3.0.0-alpha.4` per ADR-002. No sync or drift lands there.
...
| `.github/workflows/anchor-digest.yaml` | added | `workflow_dispatch`-only utility: anchors an arbitrary `quay.io/veecode/<image>@<digest>` under an operator-chosen immutable tag via `skopeo copy --all`, then verifies the tag resolves back to the same digest. Same `QUAY_USERNAME`/`QUAY_TOKEN` secrets as publish-edge's face anchor. Exists because quay GCs untagged manifests once a moving tag (e.g. `bs_1.52.0`) is republished over them — first used to preserve the M5-verified `vertigo-theme` build (`f46405d8…`) during the 2026-08-20 curation-regression investigation. |
...
| `veecode/dynamic-plugins.yaml` | added | Baked default, COPY'd since beta.6 to `dynamic-plugins.yaml.baked` while the live path `/opt/app-root/src/dynamic-plugins.yaml` became a symlink into the data volume (`/devportal-data/dynamic-plugins.yaml`, written by `merge-dynamic-plugins.js` at pre-install; with no volume/merge the link dangles and the installer warn-skips — same net zero-plugin behavior as the old baked default) — M2 D3, reworked OD1 phase B. Since OD1 phase A it also carries `includes: [dynamic-plugins.veecode.yaml]`, so a plain `docker run` (no Helm chart) still boots with the VeeCode product face instead of faceless; under Helm the chart's rendered ConfigMap replaces this file entirely, so there is no duplicate-face risk. Ported to `veecode/main` at M3.5 (the product branch going forward); the `veecode/release-1.10` copy remains, frozen at `3.0.0-alpha.4`. |
...
| `veecode/merge-dynamic-plugins.js` | added (OD1 phase B) | Level-1 config merge, staged (COPY only) the same way as `regenerate-extensions-install.js` — never invoked by any ENTRYPOINT/CMD in this repo; orchestration outside this repo chains it between that regen and the installer (`node regenerate-extensions-install.js --config <...> && node merge-dynamic-plugins.js && sh install-dynamic-plugins.sh <root>`). Reads the operator's `dynamic-plugins.yaml` from `DEVPORTAL_OPERATOR_CONFIG` (fatal, exit 1, if missing/unreadable/invalid — this is the deploy's source of truth) and the marketplace write-through from `${DEVPORTAL_DB_PATH}/extensions-install.yaml` (soft-degrades to operator-only if missing/empty), then writes `${DEVPORTAL_DB_PATH}/dynamic-plugins.yaml` (fatal without DEVPORTAL_DB_PATH; the installer's hardcoded CWD read path `/opt/app-root/src/dynamic-plugins.yaml` is a baked SYMLINK to that volume location since beta.6 — the init container runs readOnlyRootFilesystem, and the beta.5 first boot died EROFS writing to the image fs; after writing, the script realpath-asserts the symlink resolves to its output and is fatal on divergence) with `plugins: operator.plugins ++ extensions.plugins`, dropping any extensions entry whose normalized key (reuses `regenerate-extensions-install.js`'s `normalizePluginKey`) collides with an operator entry. Exists because the marketplace write-through previously entered as a level-0 `includes:` entry, colliding fatally with the catalog-index DPDY (also level 0) on any overlapping plugin key regardless of `disabled` state; landing it at level 1 instead makes a marketplace install a sanctioned cross-level override of a DPDY entry. |
| `AGENTS.md`, `CLAUDE.md` | added | Agent router; `CLAUDE.md` delegates to it. |
| `docs/upstream-drift.md` (this file) | added | Drift manifest and fork conventions. |
| `.github/workflows/update-versions.yaml` | modified | Inherited RHDH workflow, active but failing every run since it was forked: it minted its token from `RHDH_GITHUB_APP_ID`/`RHDH_GITHUB_APP_PRIVATE_KEY`, GitHub App secrets this fork does not hold, so `Generate token` failed and every later step was skipped (last such run 2026-09-14). Now uses `secrets.GITHUB_TOKEN` with an explicit least-privilege `permissions` block (`contents: write`, `pull-requests: write`) for the `create-pull-request` step. Same adaptation applied to `version-bump.yml` in devportal-plugins#156. Edit is confined to the token step and the added `permissions` block; the trigger set, the script invocation and the PR step are upstream's. M8 Stage 7. |
| `scripts/update-versions-doc.mjs` | modified | Inherited RHDH generator. It already honoured `GITHUB_REPOSITORY` when reading `backstage.json`, but wrote `redhat-developer/rhdh` into every link of the generated document, so a fork-run produced a doc pointing readers upstream. Now derives those links from the same `GITHUB_REPOSITORY` value (`repoUrl`) and defaults it to `veecode-platform/devportal-core`. Generation logic is otherwise upstream's. M8 Stage 7. |
| `docs/dynamic-plugins/versions.md` | modified (generated) | Output of `scripts/update-versions-doc.mjs`. Regenerated against this fork: Backstage 1.52.0 (was 1.49.4, stale because the workflow above never succeeded) and links resolving to this repository. Not hand-edited — regenerate rather than patch. M8 Stage 7. |
| `.rulesync/skills/devportal-release/` | added | Release skill source, in upstream's rulesync layout: carries an image through chart, index and local runner, with `scripts/release-status.sh` and `scripts/check-face-pins.sh`. Edit here, then run `npx rulesync@7.30.0 generate --features skills` and restore the scripts' executable bit. A bare `rulesync generate` deletes `AGENTS.md` and `CLAUDE.md`, because `rulesync.jsonc` sets `delete: true` and no root rule exists. |
| `.claude/skills/devportal-release/`, `.cursor/skills/devportal-release/`, `.opencode/skill/devportal-release/` | added (generated) | rulesync output of the row above; never edit by hand. |
| `packages/backend/package.json` + `packages/backend/src/index.ts` | modified (PR #11, `3.0.0-beta.8`) | GitLab webhook event validation: `@backstage/plugin-events-backend-module-gitlab@0.3.13` added to the backend deps and one `backend.add(...)` line appended immediately after upstream's own `plugin-events-backend` registration (index.ts:151). Version pinned to the release train of upstream's `plugin-events-backend@0.6.3`/`plugin-events-node@0.4.23`. **Inert by default** — the module only activates when `events.modules.gitlab.webhookSecret` is configured, the `gitlab` HTTP topic only exists under `events.http.topics`, and no catalog provider consumes events unless configured; a deployment without those keys gets no endpoint and no behavior change. If upstream RHDH ever registers this module itself, drop our line in the sync merge. Lockfile updated via `yarn --mode=update-lockfile`. |

Upstream files modified: **three** — `packages/backend/package.json` and `packages/backend/src/index.ts` (one dependency + one appended `backend.add` line, see the events-module row above), and `build/containerfiles/Containerfile`, strictly **append-only**. M2 appended four blocks after the last upstream instruction (baked `dynamic-plugins.yaml`, baked `app-config.veecode.yaml`, `ENV SEGMENT_TEST_MODE=true`, and a new single-line ENTRYPOINT retyping the upstream array plus one extra `--config`). **At M4.6 (`3.0.0-beta.1`) two of those four were removed** — the baked `app-config.veecode.yaml` COPY and the retyped ENTRYPOINT, which existed only to carry it (see the `veecode/app-config.veecode.yaml` row above). Both are replaced by explanatory comment blocks, so the append-only discipline holds and the removal is legible in place. **The fork no longer overrides upstream's ENTRYPOINT at all**, which is what `entrypoint-drift.yaml` now asserts. Live drift in this file is therefore: the baked `dynamic-plugins.yaml`, `ENV SEGMENT_TEST_MODE=true`, the M3 pre-step COPY, and — since OD1 phase A — the baked `dynamic-plugins.veecode.yaml` COPY. A fifth block (a build-generated documentation-only vitrine at `dynamic-plugins.default.yaml`) shipped in `3.0.0-alpha.2` and was removed on 2026-08-10 (D6 reversed): that filename is a live reserved placeholder in the upstream installer contract (`includes:` entry replaced via `CATALOG_INDEX_IMAGE`), and baking a file there shadows upstream behavior. M3 front 3 (Q7) appended a sixth block: `COPY veecode/regenerate-extensions-install.js` at the WORKDIR root plus a `chmod a=r`, staged after the ENTRYPOINT with no ENTRYPOINT/CMD change of its own. OD1 phase A appended a seventh block: `COPY veecode/dynamic-plugins.veecode.yaml`, alongside the existing baked `dynamic-plugins.yaml` COPY. OD1 phase B appended an eighth block, immediately after the sixth: `COPY veecode/merge-dynamic-plugins.js` plus a `chmod a=r`, same staged-but-never-invoked shape as the regen script it sits next to. No upstream line was edited or removed; staleness of the retyped ENTRYPOINT copy is guarded by `entrypoint-drift.yaml`, which reads only the last `^ENTRYPOINT` line and is unaffected by blocks appended after it.
...
Since the 2026-08-26 rename, `main` is the product branch, so every inherited
workflow that filters on `main` fires on our pushes and pull requests. The
image-building and PR-opening ones are disabled with `gh workflow disable`.
State read from `GET /repos/veecode-platform/devportal-core/actions/workflows`
on 2026-09-25:

- **Disabled:** `next-build-image.yaml` (it still targets
  `quay.io/rhdh-community/rhdh` and listens to pushes on `main`),
  `pr-build-image.yaml`, `pr-podman-push.yaml`, `pr.yaml`,
...
Never re-enable `next-build-image.yaml` or the PR image pair without first
pointing them at a VeeCode registry: they would build from `main` and push
