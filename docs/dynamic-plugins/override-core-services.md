...
## Deprecated auth provider override flag

`ENABLE_AUTH_PROVIDER_MODULE_OVERRIDE` is deprecated and ignored.

Guest and GitHub auth backend modules are loaded through dynamic plugins in the same way as other auth providers, so no override flag is required.
