...
| `veecode/dynamic-plugins.veecode.yaml` | added (OD1 phase A) | Baked VeeCode product face (Home, header, TechDocs, Notifications, Tech Radar, Marketplace, theme — 20 entries), the canonical version source for those plugins (digest-pinned OCI/npm refs; `./dynamic-plugins/dist/...` local refs carry as-is). Wired into the chart's `global.dynamic.includes` at level 0, ahead of the marketplace write-through file. **Main-only deviation from `veecode/release-1.10`**: on `release-1.10` a missing/unlisted face file fails the boot closed, enforced inside the vendorized `install-dynamic-plugins.py` (see that branch's own manifest). On `veecode/main` that script does not exist — the installer is consumed from npm (`@red-hat-developer-hub/cli-module-install-dynamic-plugins@0.4.0`, see the M3.5 re-anchor note below), which has no equivalent fail-closed/backstop guard. The `includes:` wiring still makes the face load on a healthy boot; nothing here fails the boot if it doesn't. `veecode/regenerate-extensions-install.js` still dedups the marketplace regen against this file so a face package is never re-declared at the same includes level. Ported to `veecode/main` at OD1 (image-side); the `veecode/release-1.10` copy is the original. |
