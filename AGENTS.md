...
| Path | Type | Purpose |
|---|---|---|
| `.github/workflows/upstream-sync.yaml` | added | Weekly automated merge from `upstream/main` → `main`; opens an issue on conflict. The `veecode/release-1.10` job was removed on 2026-09-25: that line is frozen at `3.0.0-alpha.4` and the job failed every week. |
...
| `packages/backend/src/modules/authProvidersModule.ts`, `rhdhSignInResolvers.ts`, `resolverUtils.ts` (+ tests), `packages/backend/package.json` auth modules | kept (upstream deleted them) | Upstream `d437e013` (#5258) removed the statically installed auth providers, including guest, and moved them to dynamic plugins from `rhdh-plugin-export-overlays`. The product face has no auth provider plugins, so we reverted that commit in the 2026-09-25 partial sync: without the static providers, guest, GitLab and OIDC sign-in stop working. Future upstream edits to these files arrive as modify/delete conflicts; drop this row and the revert once the face ships auth provider plugins. |
